...
export default function StatsScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      ...
      >
        <Text style={{ fontSize: 24, fontWeight: '800', color: c.textPrimary, marginBottom: 16 }}>학습 통계</Text>

        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
          <StatCard icon="📚" label="전체 단어" value={stats.words.total.toString()} color={c.accentBg} />
        ...
        <View style={{
          ...
          <View>
            ...
            <Text style={{ fontSize: 13, color: c.textSecondary }}>연속 학습 중</Text>
          </View>
        </View>

        {stats.wordbooks.length > 0 && (
          <View>
            <Text style={{ fontSize: 16, fontWeight: '700', color: c.textPrimary, marginBottom: 10 }}>단어장별 단어 수</Text>
            {stats.wordbooks.map((wb) => (
              <View key={wb.id} style={{
                ...
                <Text style={{ fontSize: 15, fontWeight: '600', color: c.textPrimary }} numberOfLines={1}>{wb.name}</Text>
                <Text style={{ fontSize: 13, color: c.textSecondary }}>{wb.wordCount}개</Text>
              </View>
            ))}
          </View>
        )}
