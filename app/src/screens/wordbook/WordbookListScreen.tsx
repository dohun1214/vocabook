...
import {
  ...
  RefreshControl, SafeAreaView, ActivityIndicator, KeyboardAvoidingView, Platform,
} from 'react-native';
...
export default function WordbookListScreen() {
  ...
  const fetchWordbooks = useCallback(async () => {
    try {
      const { data } = await api.get('/wordbooks');
      ...
      await Promise.all(
        wordbooks.map(async (wb) => {
          try {
            ...
            results.push(...data.words.map((w: Word) => ({ ...w, wordbookName: wb.name, wordbookId: wb.id })));
          } catch {}
        })
      );
  ...
  async function handleSave() {
    ...
    try {
      if (editTarget) {
        await api.put(`/wordbooks/${editTarget.id}`, { name: inputName.trim() });
      } else {
        await api.post('/wordbooks', { name: inputName.trim() });
      }
    ...
    Alert.alert(
      ...
      [
        { text: '취소', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await api.delete(`/wordbooks/${wb.id}`);
              fetchWordbooks();
            } catch {
